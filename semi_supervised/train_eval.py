...
def joint_cl_exp(
    ...
    for fold, (train_idx, test_idx, val_idx, semi_idx) in enumerate(
        ...
        for epoch in range(1, epochs + 1):
            model_loss = train_node_weight_view_gen_and_cls(epoch, model, optimizer, semi_loader, device)
            train_acc = eval_acc(model, semi_loader, device, with_eval_mode)
            test_acc = eval_acc(model, test_loader, device, with_eval_mode)
            train_accs.append(train_acc)
            test_accs.append(test_acc)
            
            logger.info('Epoch: {:03d}, Model Loss: {:.4f}, '
                        'Train Acc: {:.4f}, Test Acc: {:.4f}'.format(epoch, model_loss, train_acc, test_acc))
...
def train_node_weight_view_gen_and_cls(epoch, model, model_optimizer, dataloader, device):
    model_loss_all = 0
    for batch in dataloader:
        batch = batch.to(device)
        model.train()
        model.zero_grad()
        low_pass, middle_pass, high_pass, normal = model(batch, None)
        model_loss, quality  = model.calc_loss(low_pass, middle_pass, high_pass, normal, model, lambda_reg=1e-2)
        model_loss_all += model_loss.item() * batch.num_graphs

        # define classification loss
        x = low_pass
        batch.y = torch.squeeze(batch.y)
        cls_loss = F.nll_loss(x, batch.y)
        # cl_loss = loss_cl(x, x_aug)
        # model_loss = model_loss + cls_loss + cl_loss

        model_loss = model_loss + cls_loss
        model_lr = 1e-3
        max_lr = 3e-3
        min_lr = 1e-4
        if quality > 0.8:  # Example threshold for "good quality" embeddings
            new_lr = model_lr * 0.9  # Decrease learning rate
        else:
            new_lr = model_lr * 1.1  # Increase learning rate
...
def eval_acc(model, loader, device, with_eval_mode):
    ...
    for data in loader:
        data = data.to(device)
        with torch.no_grad():
            pred_tmp, _, _, _ = model(data, None)
            pred = pred_tmp.max(1)[1]
        correct += pred.eq(data.y.view(-1)).sum().item()
